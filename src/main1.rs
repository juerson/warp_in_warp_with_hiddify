...
/* 该程序：可以连续捕捉用户输入的endpoint，使用最近输入的endpoint，生成 Hiddify 配置（warp in warp） */
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let outbounds_value: json_value = serde_json::from_str(outbounds)?;
    ...
    if let Some(warp_accounts) = toml_value.get("warp_account").and_then(|v| v.as_array()) {
        ...
        loop {
            let mut hiddify_value:json_value = outbounds_value.clone();
            ...
            let formatted_json = serde_json::to_string_pretty(&hiddify_value)?;
            // 将结果复制到剪切板
